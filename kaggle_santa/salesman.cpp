...
// Fichier d'entrée sur cin.
// Sortie donnée sur cout.
...
const string method = "DIST",change = "FLIP";
// Nombre d'essais de meilleur route pour chaque cluster : on garde le meilleur essai pour chaque cluster.
const int nb_try = 1;
		...
		for(int i=0;i<n;i++)
			...
			for(int l=0;l<nb_try;l++)
			{
				...
				while(1)
				{
					++cmp;
					++cmp_iter;
					// 				cerr << cmp_iter << " " << get_score(v,total_w) << endl;
					// 				cerr << cmp_iter << " " << get_dist(v) << endl;
					int x = rand()%n,y = rand()%n;
					while((b_change && (y == x || y == (x-1+n)%n || y == (x+1)%n)) || (!b_change && y == x))
						y = rand()%n;
					
					// 				ld e1 = get_score(v,total_w),e2 = get_score_swap(v,x,y,total_w);
					ld diff;
					if(b_change)
					...
					else if(cmp > 300)
					{
						...
						for(x=0;x<n && b;x++)
							...
							// 					if(better.empty())
							// 						break;
							// 					auto p = better[rand()%better.size()];
							// 					cerr << cmp_iter << " " << get_score(v,total_w) << endl;
							// 					flip(v,p.first,p.second);
					}
					// 				if(T > eps)
					// 					T = 100/(cmp_iter+1.);
					// 				if(T <= eps)
